...
const Sidebar = ({ collapsed }) => {
  const menuItems = [
    ...
    },
    {
      ...
      subItems: [
        { title: "Open Tickets", path: "/ticketing/open" },
        { title: "Closed Tickets", path: "/ticketing/closed" },
        { title: "Ticket History", path: "/ticketing/history" },
        { title: "PDF Report", path: "/ticketing/report" }
      ]
    },
    {
      title: "Maintenance",
  ...
  return (
    <div className={`scada-sidebar ${collapsed ? 'collapsed' : ''}`}>
      <div className="sidebar-brand d-flex align-items-center justify-content-center py-4 border-bottom border-secondary border-opacity-25">
        <Activity className="text-info me-2" size={32} />
        {!collapsed && <span className="fw-bold fs-5 tracking-tight">SOCHIOT</span>}
      </div>
        ...
        {menuItems.map((item, index) => (
          item.subItems ? (
            <Accordion key={index} className="sidebar-accordion">
              <Accordion.Item eventKey={index.toString()} className="bg-transparent border-0">
      ...
      <div className="sidebar-footer mt-auto p-3 border-top border-secondary border-opacity-25">
        <NavLink 
          to="/settings" 
          className={({ isActive }) => `sidebar-link ${isActive ? 'active' : ''}`}
        >
